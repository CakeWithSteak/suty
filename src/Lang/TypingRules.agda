...
data _⊢_::_,_⨾_ (Γᵢ : TypingContext) : (t : Term α) (ty : Type) (Γₒ : TypingContext) (Ω : TypingContext) → Set where
  TUVar : (p : x ∈ α) → (x ↦ T ∈* Γᵢ) →  (qualifierOf T ≡ un) → Γᵢ ⊢ (` x # p) :: T , Γᵢ ⨾ ∅
  TLVar : (p : x ∈ α) → (x ↦ T ∈* Γᵢ) → (qualifierOf T ≡ lin) → (Γₒ : TypingContext) → Γᵢ - x ↦ T ≡ Γₒ → Γᵢ ⊢ (` x # p) :: T , Γₒ ⨾ ∅
  TOVar : (p : x ∈ α) → (x ↦ T ∈* Γᵢ) → (qualifierOf T ≡ ord) → (Γₒ : TypingContext) → Γᵢ - x ↦ T ≡ Γₒ → Γᵢ ⊢ (` x # p) :: T , Γₒ ⨾ ∅
  TAVar : (p : x ∈ α) → (x ↦ T ∈* Γᵢ) → (qualifierOf T ≡ aff) → (Γₒ : TypingContext) →  Γᵢ - x ↦ T ≡ Γₒ → Γᵢ ⊢ (` x # p) :: T , Γₒ ⨾ ∅
  TRVar : (p : x ∈ α) → (x ↦ T ∈* Γᵢ) → (qualifierOf T ≡ rel)  → Γᵢ ⊢ (` x # p) :: T , Γᵢ ⨾ (∅ , x ↦ T)
...
-- Utillity for quickly showing a contradiction when two different types are assigned to the same term
